        ...
        try:
            ...
            while not rospy.is_shutdown():  # ROS 노드가 종료될 때까지 반복
                ...
                if self.cv_image is not None:  # 카메라 이미지가 있는 경우
                    ...
                    # ls1 = cv2.getTrackbarPos('LS1', 'Trackbars')
                    # lv1 = cv2.getTrackbarPos('LV1', 'Trackbars')
                    # uh1 = cv2.getTrackbarPos('UH1', 'Trackbars')
                    # us1 = cv2.getTrackbarPos('US1', 'Trackbars')
                    # uv1 = cv2.getTrackbarPos('UV1', 'Trackbars')

                    # lh2 = cv2.getTrackbarPos('LH2', 'Trackbars')
                    # ls2 = cv2.getTrackbarPos('LS2', 'Trackbars')
                    # lv2 = cv2.getTrackbarPos('LV2', 'Trackbars')
                    # uh2 = cv2.getTrackbarPos('UH2', 'Trackbars')
                    # us2 = cv2.getTrackbarPos('US2', 'Trackbars')
                    # uv2 = cv2.getTrackbarPos('UV2', 'Trackbars')
                    ...
                    # # Filter the image to get only the desired colors
                    ...
                    # # Apply dilation to the combined mask to expand the mask area
                    ...
                    # filtered_image = cv2.bitwise_and(cropped_image_copy, cropped_image_copy, mask=mask_inv)
                    # ------- HSV 관련 --------- # 
