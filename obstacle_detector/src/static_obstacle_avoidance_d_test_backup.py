        ...
        while not rospy.is_shutdown():
            ...
            if self.state == 'L':
                if self.static_obstacle_cnt == 30:
                    ...
                    if self.gt_heading < -150:
                        self.avoid_heading = (self.gt_heading - 30) + 360
                        self.return_heading = self.gt_heading + 15

                    elif self.gt_heading > 165:
                        self.avoid_heading = self.gt_heading - 30
                        self.return_heading = (self.gt_heading + 15) - 360
                ...
                if self.gt_heading < -150:
                    if (self.heading < 0): # gt = -165 기준 heading이 -165 ~ -180 까지 우조향
                        self.angle = 1.5 * (self.heading - self.avoid_heading + 360)
                    else: # 180 ~ 165까지 더 우조향 하다가 avoid_heading 도달하면 R로 flag 변환
                        self.angle = 1.5 * (self.heading - self.avoid_heading)
                        if self.heading < self.avoid_heading:
                            self.state = 'R'

                # gt heading 165보다 크면
                elif self.gt_heading > 165:                  
                    self.angle = 1.5 * (self.heading - self.avoid_heading) #우조향
                ...
                if self.gt_heading < -150:
                    if (self.heading > 0): # gt = -165 기준 heading이 165 ~ 180 까지 좌조향
                        self.angle = self.heading - self.return_heading - 360
                    else: # -180 ~ -150까지 더 좌조향 하다가 reteun_heading 도달하면 L로 flag 변환
                        self.angle = self.heading - self.return_heading
