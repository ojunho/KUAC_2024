...
vector<double> vec_roll;
vector<double> vec_pitch;
...
int main(int argc, char** argv)
{
    ...
    vec_roll.reserve(data_count);
    vec_pitch.reserve(data_count);
    vec_yaw.reserve(data_count);
    // <<<<< Vector initialization

    // >>>>> Subscribers
    ros::Subscriber imuSub;
    imuSub = nh.subscribe<sensor_msgs::Imu>("imu",1,&processImuData);
    // <<<<< Subscribers
    ...
    DataStat stat_roll  =  calcStats(vec_roll);
    DataStat stat_pitch =  calcStats(vec_pitch);
    DataStat stat_yaw   =  calcStats(vec_yaw);
    ...
    ROS_INFO_STREAM( "Orientation Convert RPY: ( var )");
    ROS_INFO_STREAM( " * ROLL  : ( "<< roll << " )" );
    ROS_INFO_STREAM( " * PITCH : ( "<< pitch << " )" );
    ROS_INFO_STREAM( " * YAW   : ( "<< yaw << " )" );
    cout << endl;

    ROS_INFO_STREAM( "RPY: ( mean, var )");
    ROS_INFO_STREAM( " * ROLL  : ( "<< stat_roll.mean << " , " << stat_roll.var << " )" );
    ROS_INFO_STREAM( " * PITCH : ( "<< stat_pitch.mean << " , " << stat_pitch.var << " )" );
    ROS_INFO_STREAM( " * YAW   : ( "<< stat_yaw.mean << " , " << stat_yaw.var << " )" );
    cout << endl;
    // <<<<< Variance calculation
...
void processImuData( const sensor_msgs::Imu::ConstPtr& imu )
{
    ...
    vec_roll.push_back( roll );
    vec_pitch.push_back( pitch );
