...
while True:
    ...
    for i in range(boxes.size(0)):
        ...
        ymax = int(box[3])
        xmax = int(box[2])
        labels_id = int(labels[i])
        p = str(class_names[labels[i]])+','+str(prob *100)+','+str(labels_id)+','+str(ymin)+','+str(xmin)+','+str(ymax)+','+str(xmax)+'/'
        info_str += p
		
    info_str = info_str[:-1]
    print(info_str)

    # cv2.putText(orig_image,'FPS: {0:.2f}'.format(frame_rate_calc),(30,50),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,0),2,cv2.LINE_AA)
